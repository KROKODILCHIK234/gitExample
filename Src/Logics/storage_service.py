...
class storage_service:
    ...
    def create_turns_by_nomenclature(self, start_period: datetime, stop_period: datetime,
        ...
        """
            Получить обороты за период по конкретной номенклатуре
        Args:
            start_period (datetime): Начало
            stop_period (datetime): Окончание
        ...
        # Фильтруем
        prototype = storage_prototype(self.__data)
        filter = prototype.filter_by_period(stop_period, start_period)  # Reversed the start and stop period here.
        filter = filter.filter_by_nomenclature(nomenclature)
        if not filter.is_empty:
            raise operation_exception(f"Невозможно сформировать обороты по указанным данным: {filter.error}")
